...
<!DOCTYPE html>
    <head>
        <title>Order Report | Chick Cafe</title>
    ...
    <body>
        ...
        <nav>
            <ul>
                ...
                $r = new report();
                $dis = $r->oReportPDF($_POST['startDate'],$_POST['endDate']);
                $_SESSION['report'] = $dis;
                echo $_SESSION['report'];

                        $date = date('Y-m-d H:i:s');
                        $q = "INSERT INTO `reports`(`idmanager`, `date`, `type`) VALUES ('$_SESSION[id]','$date','Order')";
                        $sth = $db->prepare($q);
                        $execute = $sth->execute();
             }
                ?>

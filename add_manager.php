...
<!DOCTYPE html>
    <head>
        ...
        <title>Add Manager | Chick Cafe</title>
    ...
    <?php
        ...
        echo '<tr><td><p>Owner:</p></td><td><input name="owner" type=checkbox value="owner"/></td></tr>';
    ?>  
        ...
        if(isset($_POST['owner']) && $_POST['owner'] == 'owner'){
            ...
            if(isset($_POST['name'])&&isset($_POST['pass'])&&isset($_POST['email'])){
                if ($_POST['pass']!=$_POST['pass2']){   //check if the passwords match
                    ...
                    echo '<META HTTP-EQUIV="Refresh" Content="0; URL=add_manager.php">';
                    die();
                }
                ...
                if($count[0]['ct'] == 1){
                    ...
                    $question = "INSERT INTO manager(owner,name,password,email,active) VALUES (1,:name, :pass, :email, :active)";
            ...
            if(isset($_POST['name'])&&isset($_POST['pass'])&&isset($_POST['email'])){
                ...
                if($count[0]['ct'] == 1){
                    ...
                    //if it doesnt exist insert the new employee details into the table
                    $question = "INSERT INTO manager(owner,name,password,email,active) VALUES (0,:name, :pass, :email, :active)";
                    $add = $db->prepare($question, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
                    $add->execute(array(':name' => $name, ':pass' => $encrypt_password, ':email' => $email, ':active' => true));
                    echo '<META HTTP-EQUIV="Refresh" Content="0; URL=manage_managers.php">'; 
                }

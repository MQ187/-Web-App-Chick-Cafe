<?php
	...
	if ($_POST['password']!=$_POST['password2']){
			...
			header("Location:new_manager.php");
			die();
		}
	...
	if($count[0]['ct'] == 1){
		...
		$question="INSERT INTO employee(owner,name,password,email,active) VALUES(:owner, :name, :password, :email, :active)";
	
		$add = $db->prepare($question, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));

		$add->execute(array(':owner' => false, ':name' => $name, ':password' => $encrypt_password, ':email' => $email, ':active' => true));
